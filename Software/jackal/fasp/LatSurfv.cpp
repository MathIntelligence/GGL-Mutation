...
void LatSurfv::ready(Pdb *a, int spherelevel,float pro) {
Atom **aa=0;
aa=a->getAtomAll();
ready(aa,spherelevel,pro);
}

void LatSurfv::ready(Atom**a,int spherelevel,float pro) {
...
float LatSurfv::Surface(float *center,float rad) {
Vector v;
...
return Surface(v,rad);
}


float LatSurfv::Surface(Vector center,float rad)
{
int j,k,l,nacc;
int iv,ie,ifc,ivm;
...
float LatSurfv::Surface(float x,float y,float z,float rad) {
 float xyz[3];
 xyz[0]=x;
 xyz[1]=y;
 xyz[2]=z;
 return Surface(xyz,rad);
}

float LatSurfv::calcarea(Atm *s) {
	Atom **ss=atom;
	int i=s->id0;
	float ta=Surface(ss[i]->pos.x,ss[i]->pos.y,ss[i]->pos.z,ss[i]->radius+probe_radius);
        s->area=ta;
	s->mask|=TRES.constant->surfacecalculated;
	return ta;
}

float LatSurfv::calcarea(Res *s) {

	float area=0;
	for(Atm *a=s->atm;a;a=a->next) 
	{
		area+=calcarea(a);
	}
	return area;
...
float LatSurfv::calcarea(Res *s,int n) {
	float area=0;
	for(Res *a=s;a;a=a->next) {
		if(a->id0>n) break;
		area+=calcarea(a);
	}
	return area;
...
float LatSurfv::calcarea(Chn *s) {

        float area=0;
        for(Res *a=s->res;a;a=a->next) 
        {
                area+=calcarea(a);
        }
        return area;
...
float LatSurfv::calcarea(Pdb *s) {

        float area=0;
        for(Chn *a=s->chn;a;a=a->next)  
        {
                area+=calcarea(a);
        }
        return area;
...
void LatSurfv::update(Atm *s) {
	atom[s->id0]->pos.x=s->xyz[0];
	atom[s->id0]->pos.y=s->xyz[1];
	atom[s->id0]->pos.z=s->xyz[2];
	atom[s->id0]->radius=s->tatm->eng->radius;
}

void LatSurfv::update(Res *s) {
...
void LatSurfv::update(Res *s,int n) {
        for(Res *a=s;a;a=a->next) {
                if(a->id0>n) break;
                update(a);
        }
...
void LatSurfv::clearsurfvmask(Atm *s) {
        s->mask&=~TRES.constant->surfacecalculated;
}

void LatSurfv::clearsurfvmask(Res *s) {
...
void LatSurfv::clearsurfvmask(Res *s,int n) {
        for(Res *a=s;a;a=a->next) {
                if(a->id0>n) break;
                clearsurfvmask(a);
        }
...
void LatSurfv::clearsurfvmask(Chn *s) {
...
void LatSurfv::clearsurfvmask(Pdb *s) {
...
void LatSurfv::assignarea(Pdb *s) {
