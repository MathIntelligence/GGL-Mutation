...
void ProFix::create(Res *s,int n)
{
  if(segment) delete segment;segment=0;
  segment=new Pdb;
  segment->chn=new Chn;
...
Pdb* ProFix::mycreate(Res *s,int n)
{
  Pdb *segm;
  segm=new Pdb;
  segm->chn=new Chn;
...
void ProFix::randmzHelix(int rand)
{
  Res *r;
  Atm *a,*a0;
  int j,i,n;
...
int ProFix::randmzChiangle(int nu)
{
  Res *r;
  Atm *a,*a0;
  int i,n;
...
int ProFix::randmzChianglefix(int ang1,int ang2)
{
  Res *r;
  Atm *a,*a0;
  Rotate rot;
...
void ProFix::randmzSheet(int rand)
{
  Res *r;
  Atm *a,*a0;
  int j,i,n;
...
float ProFix::closeg(ProFix *seg1,ProFix *seg2)
{ 
  ProFix *segen[2];
  Res *r,*last,*last0,*rr;
  Atm *aa[2],*aa0;
...
void ProFix::writeseg(float **xyz_all,char *s,int nn)
{
  int i,all;
  FILE *fp;
  char nam[1000];
...
void ProFix::write(float **xyz_all,char *s,int nn)
{
  Res *r0,*r;
  int i,nseq,all;
  FILE *fp;
...
float **ProFix::predtsave(int flg)
{
Res *r,*r0;
float **xyz_all,*xyz,*ent;
int i,j,kep,nseq;
...
float ProFix::rmsdmutate(int flg)
{
Res *r,*rr;
Atm *a,*aa;
int i,j;
...
float ProFix::getenergy(Res *s,int to) {
	Res *t=(*s->chn)[to];
...
float ProFix::clashold(Res *s,char *force) 
{ 
  Atm *a; 
  float e;
  e=0;
...
float ProFix::clash(Res *s,int to)
{
  Res *r;
  float e;
  e=0;
...
float ProFix::clash(Res *s,int from,int to)
{
  Atm *a;
  float e;
  e=0;
...
float ProFix::clash(Res *s) //original
{ 
  Atm *a; 
  float e;
  e=0;
...
float ProFix::clashold(Atm *aa0,char *force)
{
  Atm *aa1,*all[50];
  float dr,dn,e,d,elon,xr[3];
  int i,j,m,n,isp;
...
float*  ProFix::scap(float **xyz_all)
{
 Res *r,*r0;
 int nseq;
 float d,*xyz;
...
float*  ProFix::scapfix(float **xyz_all)
{
 Res *r,*r0;
 int nseq;
 float *xyz;
...
float ProFix::minimize(float *xyz0,int flg)
{
  float **xyz_all;
  float *ent,d;
  xyz_all=new float*[2];
...
float ProFix::minimizefix(float *xyz0,int flg)
{
  float **xyz_all;
  float *ent,d;
  xyz_all=new float*[2];
...
float* ProFix::minimize(float **xyz_all,int flg)
{
  Res *r0,*r;
  float *xyz;
  int i,nseq,all;
...
float* ProFix::minimizefix(float **xyz_all,int flg)
{
  Res *r0,*r;
  float *xyz;
  int i,nseq,all;
...
float* ProFix::boundminimizefix(float **xyz_all,int flg)
{
  Res *r0,*r;
  float *xyz;
  int i,nseq,all;
...
float* ProFix::minimizesidechain(float **xyz_all,int flg)
{
  Res *r0,*r;
  float *xyz;
  int i,nseq,all;
...
float** ProFix::zipper(float dis,int flg)
{
...
float** ProFix::cross(float **xyz_all,int cot,float cut)
{
  //loop cross
  Chn *cht;
  Res *r,*r0;  
 ...
 //space
  ...
  seg1=new ProFix(this); seg2=new ProFix(this);
...
float* ProFix::ensortold(float **xyz_all,char *flg)
{
  Res *r,*r0;
  Atm *a,*a1;
  int i,j,nseq,all,m;
...
float* ProFix::ensort(float **xyz_all)
{
  Res *r,*r0;
  Atm *a;
  int i,j,nseq,all;
  ...
  */
  ...
  if(strchr(dforce,'r')) mui=1;
  for(i=0;mui&&i<all;i++) {	
  	chn->transfer(xyz_all[i],r0,end,1);  
	Chn *chn0=pdborg->ischain(chn->id);
	float rmsd=0;
	int nmat=0;
	for(r=r0;r;r=r->next) {
		if(r->id0>end) break;
	 	Res *t=chn0->isresoid(r->oid);
		Atm *a,*b;
		for(b=t->atm;t;t=t->next) {
			int weak=0;
			if(rbackbone&&b->tatm->name[1]!='H'&&b->tatm->id>4) weak=1;
			if(rbackbone&&b->tatm->name[1]=='H'&&b->tatm->bond[0]->id>3) weak=1;
			a=r->isatmid(b->tatm->id);
	
			if(a==0) continue;
			if(weak==0) {
			rmsd+=TRES.distsqr(a->xyz,b->xyz);
			}
			else {
			rmsd+=0.1*TRES.distsqr(a->xyz,b->xyz);	
			}
			nmat++;
		}
...
float ProFix::calcavgrmsd(float **xyz_all,int num) {
...
float*ProFix::calcenergycoeff(float *ent,int num) {
...
float ProFix::noclose(float **xyz_all,float *xyz0) 
{
  Res *r0,*r;
  int nseq,j,n,all;
  float d,*xyz,close;
...
int ProFix::noclose(float **xyz_all,float close)
{
  Res *r0,*r;
  int nseq,i,j,m,all;
  float *pot,d,*xyz; 
...
int ProFix::nolarge(float **xyz_all,float close)
{
  Res *r0,*r;
  int nseq,i,j,m,all;
  float *pot,d,*xyz; 
...
float **ProFix::around(float **xyz_all)
{
  float *ent;
...
float *ProFix::expand(float **xyz_all,int flg)
{
  Res *r0,*r;
  int nseq;
  int i,kep;
...
float ProFix::expand(float *xyz0,int flg)
{
  Res *r0,*r;
  float d,*ent,**xyz_all;
  int kep,i,n;
...
float** ProFix::expand(float *xyz0,int seed,int kep,int ang)
{
...
float *ProFix::peel(float **xyz_all,int got)
{
...
float **ProFix::refill(float **xyz_all,int need)
{
  Res *r,*r0;
  float *ent;
  float **xyz_all0;
...
float **ProFix::fill(float **xyz_all,float cut)
{
  Res *r0,*r;
  int i,kep,kep0,nseq;
  float **xyz_all0,*xyz;
...
float **ProFix::copy(float **xyz_all)
{
  int kep;
  kep=0;
  while(xyz_all[kep])kep++;
...
float **ProFix::copy(float **xyz_all,int all)
{
  Res *r0,*r;
  float **xyz_temp;
  int  i,kep,nseq;
...
float* ProFix::trim(float **xyz_all,float*ent,int haa)
{
  int  i,kep,kk;
  float e,d;
  float **pot,*entt;
...
float* ProFix::trim(float **xyz_all,float *ent,float cutf)
{
 int i,kk,kep;
 kep=0;
 while(xyz_all[kep])kep++;
...
float* ProFix::trimold(float **xyz_all,int haa)
{
 int kep,i;
...
float* ProFix::trim(float **xyz_all,int haa)
{
 int kep,i;
 float e,rms,rms0,*ent,*entt;
 char dforce0[100];
...
float* ProFix::trim(float **xyz_all,int haa,int fe)
{
 int kep,i;
 float e,rms,rms0,*ent,*entt;
 char flg0[100];
...
void ProFix::setoff(float **xyz_all,float ct)
{
    Res *r,*r0;
    Atm *a;
    float d;
...
float **ProFix::generateold(float **xyz_all,int cop,float close)
{
...
float **ProFix::generate(float **xyz_all,int cop,float close)
{
   float *ent;
   int i,kep,kep0;
...
//preparing lattice
  ...
  for(rec=0;rec<cycle;rec++)
  {
    ...
    {
       d=lamba[i];
       if(d>step)d=step;//*(random()%10)*0.1;
       else if(d<-step) d=-step;//*(random()%10)*0.1;
       unknown[i]=d;
    }
...
float ProFix::boundmin(int fast)
{
  Res *r,*r0,*r1,*last,*last0;
  Atm *aa2,*aa0,*aa1,*all[50];
  float dn,dca,d,dmin;
  ...
  for(rec=0;rec<cycle;rec++)
  {
	...
	float a1;
		...
		for(bt=bound->tag[2*aa0->id0];bt<=bound->tag[2*aa0->id0+1];bt++) {
			...
			int weak=0;
			if(rbackbone&&aa0->tatm->name[1]!='H'&&aa0->tatm->id>4) weak=1;
			if(rbackbone&&aa0->tatm->name[1]=='H'&&aa0->tatm->bond[0]->id>3) weak=1;
   ...
   if(dace&&dace->more) {	 
    ...
    if(qut&&fast==1&&fabs(exp1st[mseq]-dmin)<0.001&&dcut<outlet) break;
    ...
    */
    ...
    for(r=r0;r;r=r->next)
    {
      ...
      for(aa2=r->atm;aa2;aa2=aa2->next)
      {
        ...
        for(i=0;i<aa2->tatm->nbond;i++) if(aa2->bond[i]) j++;
        if(j<=1) continue; //kick out no children's case torsion angle
	if(aa2->bond[0]==0) continue;
        aa1=(*r1)[aa2->tatm->id]; 
        d=unknown[m]*57.29578;
        if(d==0) {m++;continue;}
	if(aa1->tatm->id==0) rot.rotateomega(aa1,d,last->id0,direct);
        else if(aa1->tatm->id<3)rot.rotate(aa1,d,last->id0,direct);
        else rot.rotate(aa1,d);
        m++;
      }
    }
...
float ProFix::sidechainmin(int fast)
{
  Res *r,*r0,*r1,*last,*last0;
  Atm *aa2,*aa0,*aa1,*all[50];
  float dn,dca,d,dmin;
...
void ProFix::checkomega(FILE *fp) {
for(Res *r=segment->chn->res;r;r=r->next){
        Atm *a=r->isatmid(0);
...
int ProFix::linkviaboth(int first,int last,int mid) {
	Rotate rot;
	start=first;
	end=last;
	direct=1; 
...
void ProFix::updatexyzsave(int *stem) {
...
float *ProFix::myfixsegment(int *stem) {
...
float *ProFix::fixpdbsegment(Res **stem) {
...
float **ProFix::newfixsegment(int *stem) {
...
int ProFix::fixsegment(int *stem) {
...
int ProFix::resort(SegBed *segbed) {
...
void ProFix::setpdbfixbound(int flg) {
	...
	chn0=pdborg->ischain(cid); 	 
	for(r1=r0;r1&&nte;r1=r1->next) {
		if(r1->id0>end) break;
		r=chn0->isres0(r1->id0);
		if(r==0) continue;
		...
		for(a=r1->atm;a;a=a->next) {
			...
			bound->addbounds(b->id0,a->id0,0,0,dd,1);
...
void ProFix::setbound(int flg,int flg0) {
...
float **ProFix::zipperallpossible(){
...
float **ProFix::zipperit(){
  	 
	return zipperboth(discut*(end-start+1)*100);	 	 
}
float **ProFix::zipperit(float d){
  	 
	return zipperboth(d);	 	 
}
float **ProFix::addup(float **x,float **y) {
...
float **ProFix::indavgminimize(float **xyz_all) {
...
float *ProFix::avgminrmsd(float **xyz_all,float *avg) { 
...
float **ProFix::avgminimize(float **xyz_all) {
...
float **ProFix::allavgminimize(float **xyz_all) {
...
void ProFix::clearxyz(float **xyz_all,int n) {
...
float **ProFix::cutlargermsd(float **xyz_all,float rcut) {
...
float **ProFix::cutlargermsd(float **xyz_all,float rcut,int ntot) {
...
{ 
  ...
  if(TRES.logg>3) {
	float *xyz0=chn->gettransfer(r0,end);
	if(onlyxyz==1) strcpy(dforce,"r");  
	else if(onlyxyz==0) strcpy(dforce,"r");  
	ent=ensort(xyz_all); 
  	kep=0;while(xyz_all[kep])kep++;
  	for(i=0;i<kep;i++) {
     		chn->transfer(xyz_all[i],r0,end,1);	
  ...
  //constrained minimization on loops
  if(1) {
	step=0.17;numclose=50;cycle=50;flat=1; 	 
  ...
  //minimize again only on sidechain
  if(0) {
	step=0.17/2;numclose=50;cycle=50;flat=1;
	onlysidechain=1;
	strcpy(dforce,"r");
	ent=minimizefix(xyz_all,1);if(ent) delete [] ent; ent=0;	
	onlysidechain=0;
...
float **ProFix::getnewxyz(float **xyz_all,Res *ss,int nn){
...
void ProFix::readyminfix(Res *s,int n) {
...
float **ProFix::minfix(float **xyz_all) {
...
int ProFix::findmidresidue(int a,int b) {
...
int ProFix::findmidresidue(Res *a,Res *b) {
...
Res *ProFix::getmidresidue(Res *r1,Res *r2) {
...
float** ProFix::zipperboth(float dis)
{
  //generate loop conformations
  ProFix seg1,seg2;
  Res  *r1,*r2;
...
float** ProFix::zipperfix(float dis,int flg)
{
  //generate loop conformations
  Res  *r1,*r2;
  Res  *r0,*t0,*r;  
...
float** ProFix::zipperfix0(float dis,int flg)
{
  //generate loop conformations
  Res  *r1,*r2;
  Res  *r0,*t0,*r;  
  ...
  r1=(*chn)[end];r2=(*segment->chn)[end]; 
	...
	r1=(*chn)[start];r2=(*segment->chn)[start];
  }
  //decides the segment to be kept
...
void ProFix::randmzfix(ProFix *seg1,ProFix *seg2)
{
  Res *r,*r1,*r2,*r3;//,*r5;
  Res *last;
  //Atm *a;
...
float *ProFix::getaveragepost(float **xyz_all) {
...
int ProFix::ifrotatmexist(Atm *aa2) {
