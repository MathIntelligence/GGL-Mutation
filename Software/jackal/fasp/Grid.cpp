...
Grid::Grid(Atom **data,float sz,int le) 
{
int i,j,k,l,o;
...
{
cerr<<"Grid::Grid(), too many grid boxes"<<endl;
}
...
ObjectBox& Grid::operator[](Vector v)
{
int i, j, k;
...
int t=0;
for(int j=0;j<ne;j++) {
//cerr<<i<<"  "<<j<<" "<<element[j]<<endl;
if(element[j]==i) continue;
element[t++]=element[j];
}
//if(ne!=t) cerr<<i<<"  found one: "<<t<<" "<<ne<<endl;
ne=t;
if(ne==0){
if(element) delete [] element;
element=0;max=30;ne=0;
}
}

int ObjectBox::exist(int i) {
int t=0;
for(int j=0;j<ne;j++) {
