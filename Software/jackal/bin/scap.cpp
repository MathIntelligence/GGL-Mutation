...
for(n=1;n<argc;n+=2)
{
  ...
  else if(!strcmp(argv[n],"-dsqrt"))//minimization
  {
   int nt=atoi(argv[n+1]);
   scprd.dsqrt=nt;
  }
...
for(c=pdb.hetatm;c;c=c->next) {
	...
	for(a=r->atm;a;a=a->next) {
		cerr<<c->id<<" "<<r->tres->name3<<" "<<r->oid<<" "<<a->name<<" "<<r->id0<<" "<<a->id<<" "<<a->id0<<" "<<a->xyz[0]<<" "<<a->xyz[1]<<" "<<a->xyz[2]<<" "<<a->tatm->eng->radius<<" "<<a->tatm->eng->epslon<<" "<<a->tatm->eng->charge<<endl;
	}
}
...
for(a=t->tatm;a;a=a->next) {
  cerr<<t->id<<" "<<t->name3<<" "<<a->name<<" "<<a->xyz[0]<<" "<<a->xyz[1]<<" "<<a->xyz[2]<<" "<<a->eng->radius<<" "<<a->eng->epslon<<endl;
}
}
}
